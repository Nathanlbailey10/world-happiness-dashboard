...
def build_trend_tables(start_year, end_year):
    ...
    country_trends = (
        trend_data
        .groupby(["Country_Key", "Year"], as_index=False)[TREND_VARIABLES]
        .mean()
    )

    region_trends = (
        trend_data
        .groupby(["Geographic_Group", "Year"], as_index=False)[TREND_VARIABLES]
        .mean()
    )
...
geographic_groups = sorted(
    df["Geographic_Group"].dropna().unique()
)
...
group_range = [
    COLOR_PALETTE.get(group, "#999999")
...
if geographic_group:
    subregion_options = sorted(
        df.loc[
            df["Geographic_Group"] == geographic_group,
            "Region_Standardized"
        ]
        .dropna()
        .unique()
...
country_map_data = country_map_data.dropna(
    subset=["country_code"]
).copy()
...
if not country_map_data.empty:
    ...
    map_happiness_min = float(
        country_map_data["avg_happiness"].min()
    )

    map_happiness_max = float(
        country_map_data["avg_happiness"].max()
    )
...
if country_map_data.empty:
    ...
    world_data = country_map_data.to_dict(
        orient="records"
    )
    ...
    map_spec = {
        ...
        "description": (
            "Interactive world happiness choropleth "
            "with pan and zoom."
        ),
        "width": 900,
        "height": 520,
    ...
    <div class="world-happiness-map-shell">
        <div class="world-happiness-map-area">
            <div id="world-happiness-map"></div>
        </div>

        <div
            class="world-happiness-legend"
            aria-label="Happiness score legend"
        >
            <div class="world-happiness-legend-row">
                <div class="world-happiness-legend-label">
                    Higher happiness
                </div>
        ...
        <div
            id="world-happiness-tooltip"
            class="world-happiness-tooltip"
        ></div>
    </div>
            ...
            tooltip.innerHTML =
                ...
                rows.map(
                    ([label, rowValue]) =>
                        ...
                        '<span class="world-happiness-tooltip-label">' +
                        label +
                        '</span>' +
                        '<span class="world-happiness-tooltip-value">' +
                        rowValue +
                        '</span>' +
                        '</div>'
                ).join('');
    ...
    st.caption(
        "Tip: hold Ctrl on Windows/Linux or Cmd on Mac while scrolling to zoom; drag to pan."
    )
...
if geographic_group:
    country_pool = country_pool[
        country_pool["Geographic_Group"] == geographic_group
    ]

if subregion:
    country_pool = country_pool[
        country_pool["Region_Standardized"] == subregion
    ]

country_options = sorted(
    country_pool["Country_Key"]
    .dropna()
    .unique()
    .tolist()
)
...
if geographic_group:
    region_avg = (
        trend_data[
            trend_data["Geographic_Group"] == geographic_group
        ]
        .groupby("Year", as_index=False)["Happiness score"]
        .mean()
        .rename(
    ...
    if not region_avg.empty:
        region_color = COLOR_PALETTE.get(
            geographic_group,
            "#999999"
        )
...
if subregion:
    subregion_avg = (
        trend_data[
            trend_data["Region_Standardized"] == subregion
        ]
        .groupby("Year", as_index=False)["Happiness score"]
        .mean()
        .rename(
    ...
    if not subregion_avg.empty:
        ...
        subregion_color = subregion_color_map.get(
            geographic_group,
            "#BBBBBB"
        )
...
if start_year >= end_year:
    st.warning(
        f"Select a different end year to compare with {start_year}."
    )
    ...
    if geographic_group:
        filtered = filtered[
            filtered["Geographic_Group"] == geographic_group
        ]

    if subregion:
        filtered = filtered[
            filtered["Region_Standardized"] == subregion
        ]
    ...
    if change_data.empty:
        st.error(
            "No matching countries found for this selection."
        )
        st.stop()

    change_data["Happiness Change"] = (
        change_data["Happiness_End"] -
        change_data["Happiness_Start"]
    )

    change_data["Change Direction"] = (
        change_data["Happiness Change"]
        .apply(
            lambda value: (
                "Increase"
    ...
    st.subheader(
        f"Changes from {start_year} to {end_year}"
    )
    ...
    with top_left:
        st.subheader(
            "Largest Happiness Changes"
        )

        st.caption(
            "Click a bar to select a country. "
            "Double-click empty space to clear the selection."
        )

        if "n_countries" not in st.session_state:
            st.session_state.n_countries = 10

        biggest_drops = change_data.nsmallest(
            st.session_state.n_countries,
            "Happiness Change"
        )

        biggest_gains = change_data.nlargest(
            st.session_state.n_countries,
            "Happiness Change"
        )
        ...
        bar_chart = (
            ...
            .encode(
                ...
                tooltip=[
                    ...
                    alt.Tooltip(
                        "Happiness_Start:Q",
                        title=f"Happiness {start_year}",
                        format=".2f"
                    ),
                    alt.Tooltip(
                        "Happiness_End:Q",
                        title=f"Happiness {end_year}",
                        format=".2f"
                    ),
                    alt.Tooltip(
                        "Happiness Change:Q",
        ...
        if isinstance(
            ...
            if (
                selected_payload is None and
                "country_select" in selection_state
            ):
                selected_payload = (
                    selection_state.get(
                        "country_select"
    ...
    with top_right:
        st.subheader(
            "Indexed Factor Trends"
        )

        st.caption(
            "Click legend items to show or hide metrics; "
            "the chart rescales automatically."
        )
        ...
        if trend_long.empty:
            ...
            trend_long["Indexed Change"] = (
                trend_long["Value"] -
                trend_long["Baseline"]
            )

            trend_long["Metric Short"] = (
                trend_long["Metric"]
                .map(short_labels)
            )
            ...
            factor_select = alt.selection_point(
                ...
                value=[
                    ...
                    for metric in metric_short_order
                ]
            )
...
if geographic_group:
    correlation_data = correlation_data[
        correlation_data["Geographic_Group"] ==
        geographic_group
    ]

if subregion:
    correlation_data = correlation_data[
        correlation_data["Region_Standardized"] ==
        subregion
    ]
...
correlation_data = correlation_data.dropna(
    subset=correlation_variables
)

if correlation_data.empty:
    st.error(
        "No data available for the selected correlation filters."
    )
    st.stop()
...
corr_happiness["Short Label"] = (
    corr_happiness["Variable"]
    .map(short_labels)
)
...
x_variable = st.selectbox(
    ...
    [
        variable
        for variable
        in correlation_variables
        if variable != "Happiness score"
    ],
    index=None,
    placeholder=(
...
y_variable = st.selectbox(
    "Y-axis variable",
    correlation_variables,
    index=correlation_variables.index(
        "Happiness score"
    )
)
...
if x_variable is None:
    st.info(
        "Select an X-axis variable above to explore "
        "the relationship with Happiness score."
    )
